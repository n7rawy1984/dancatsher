...
export function WhatsAppContact({ locale }: { locale: Locale }) {
  ...
  useEffect(() => {
    ...
    function check() {
      ...
      const modalOpen = Boolean(document.querySelector('dialog[open], .mobile-nav:not([hidden])'));
      const obstacles = Array.from(
        document.querySelectorAll(
          'main a, main button, main input, main select, main textarea, main form, main p, main h1, main h2, main h3, footer a',
        ),
      ).map((element) => element.getBoundingClientRect());
      const safeBottom = [16, 80, 144].find((offset) => {
        const top = window.innerHeight - offset - box.height;
        return !obstacles.some(
          (rect) =>
            rect.width > 0 &&
            rect.height > 0 &&
            rect.left < box.right + 8 &&
            rect.right > box.left - 8 &&
            rect.top < top + box.height + 8 &&
            rect.bottom > top - 8,
        );
      });
      setObstructed(modalOpen || safeBottom === undefined);
      if (safeBottom !== undefined) setBottom(safeBottom);
    }
    function schedule() {
  ...
  return (
    <a
      ...
      title={label}
      style={{
        visibility: obstructed ? 'hidden' : 'visible',
